...
namespace CodingChallenges
{
    ...
    public class RunnerResult<TInput, TOutput>
    {
        ...
        public TimeSpan? ElapsedTime { get; set; }
        public bool Completed { get; set; }
        public bool IsOk => Completed && ElapsedTime != null && ElapsedTime.Value <= TargetTime * 1.2 && ExpectedOutput.Equals(Output);

        public static string PrintSummary(IEnumerable<RunnerResult<TInput, TOutput>> results)
        {
            var count = Convert.ToDouble(results.Count());
            var ok = results.Count(r => r.IsOk);
            if (ok == 0)
        ...
        public string Print()
        {
            return $"[{ElapsedTime?.Ticks ?? -500} ticks][target {TargetTime.Ticks}][Result: {Output?.ToString() ?? "No output"} == {ExpectedOutput} :Expected][{(IsOk ? " OK" : "NOK")}]";
        }
